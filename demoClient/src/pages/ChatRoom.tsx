      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            <h3 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: 18, fontWeight: 700, color: "#f0f0f0", margin: "0 0 8px" }}>
              Leave Room?
            </h3>
            <p style={{ color: "rgba(255,255,255,0.45)", fontSize: 14, lineHeight: 1.6, margin: "0 0 24px" }}>
              You're about to leave <strong style={{ color: "rgba(255,255,255,0.7)" }}>{roomName}</strong>. You'll need an invite code to rejoin.
            </p>
...
export default function ChatRoom() {
  ...
  function sendMessage() {
    ...
    if (!trimmed) return;
    socket.emit("message:send", { username, roomId: Number(roomId), content: trimmed });
    setText("");
  ...
  function handleLeaveConfirm() {
    // TODO: call leaveRoomApi(roomId) once you build it
  ...
  for (const msg of messages) {
    ...
    if (last && last.senderName === msg.senderName) {
      ...
      groupedMessages.push({ senderName: msg.senderName, messages: [msg], isOwn });
    }
  }
  ...
  return (
    <div style={{
      ...
      >
        ...
        {!roomInfo && <div className="skeleton" style={{ width: 38, height: 38, borderRadius: 12, flexShrink: 0 }} />}
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          {roomInfo ? (
            <>
              <p style={{
                fontFamily: "'Space Grotesk',sans-serif", fontWeight: 700,
                fontSize: 15, color: "#f0f0f0", margin: 0,
                whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
              }}>
                {roomInfo.name}
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 1 }}>
                <span style={{ color: "rgba(255,255,255,0.3)", display: "flex" }}><HashIcon /></span>
                <span style={{ color: "rgba(255,255,255,0.3)", fontSize: 12, fontFamily: "monospace", letterSpacing: "0.05em" }}>
                  {roomCode}
                </span>
              </div>
            </>
            ...
            <>
              <div className="skeleton" style={{ height: 14, width: 120, marginBottom: 5 }} />
        ...
        <div ref={menuRef} style={{ position: "relative", flexShrink: 0 }}>
          <motion.button
            ...
            style={{
              background: isMenuOpen ? "rgba(255,255,255,0.09)" : "rgba(255,255,255,0.05)",
              border: "1px solid rgba(255,255,255,0.09)",
              borderRadius: 10, cursor: "pointer",
            ...
            {isMenuOpen && (
              ...
              >
                ...
                <button
                  ...
                  onClick={() => { copyRoomCode(); setIsMenuOpen(false); }}
                >
                ...
                <button className="menu-item normal" onClick={() => setIsMenuOpen(false)}>
                  <span style={{ color: "rgba(255,255,255,0.35)" }}><InfoIcon /></span>
                  <span>Room info</span>
                  <span style={{ marginLeft: "auto", fontSize: 11, color: "rgba(255,255,255,0.2)" }}>Soon</span>
                </button>
      ...
      >
        ...
        {isLoadingMessages && (
          <>
            {[80, 140, 60, 200, 100].map((w, i) => (
              <div key={i} style={{ display: "flex", alignItems: "flex-end", gap: 10, justifyContent: i % 2 === 0 ? "flex-start" : "flex-end" }}>
                {i % 2 === 0 && <div className="skeleton" style={{ width: 32, height: 32, borderRadius: "50%", flexShrink: 0 }} />}
                <div className="skeleton" style={{ height: 40, width: w, borderRadius: 14 }} />
              </div>
            ))}
          </>
        )}
        ...
        {!isLoadingMessages && messages.length === 0 && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            style={{ margin: "auto", textAlign: "center", padding: "40px 20px" }}
          >
            <div style={{
              ...
              display: "flex", alignItems: "center", justifyContent: "center",
              color: "rgba(110,231,247,0.4)", margin: "0 auto 16px",
            }}>
              <ChatIcon />
            </div>
            <p style={{ color: "rgba(255,255,255,0.35)", fontSize: 15, fontWeight: 500, margin: "0 0 6px" }}>
              No messages yet
            </p>
            <p style={{ color: "rgba(255,255,255,0.18)", fontSize: 13, margin: 0 }}>
        ...
        {/* Message groups */}
        {!isLoadingMessages && groupedMessages.map((group, gi) => (
          <motion.div
            key={gi}
            initial={{ opacity: 0, y: 10 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            <div style={{ maxWidth: "70%", display: "flex", flexDirection: "column", gap: 2, alignItems: group.isOwn ? "flex-end" : "flex-start" }}>
              {/* Sender name */}
              {!group.isOwn && (
                <span style={{
              ...
              {group.messages.map((msg, mi) => (
                <div
                  ...
                  style={{
                    borderTopLeftRadius: !group.isOwn && mi === 0 ? 5 : undefined,
                    borderTopRightRadius: group.isOwn && mi === 0 ? 5 : undefined,
                  }}
                >
                  {msg.content}
                </div>
      ...
      >
        <div style={{
          ...
          onFocusCapture={(e) => {
            (e.currentTarget as HTMLDivElement).style.borderColor = "rgba(110,231,247,0.35)";
            (e.currentTarget as HTMLDivElement).style.boxShadow = "0 0 0 3px rgba(110,231,247,0.07)";
          }}
          onBlurCapture={(e) => {
        ...
        <p style={{ margin: "6px 0 0", textAlign: "center", fontSize: 11.5, color: "rgba(255,255,255,0.15)" }}>
